    ...
    >
      ...
      <ul className="space-y-3">
        {tracks.map((track, index) => (
          ...
          >
            <span className="mr-2 md:mr-3 text-purple-300 opacity-75">{index + 1}.</span>
            <div className="flex-grow">
              <a 
                href={`https://www.youtube.com/results?search_query=${track.searchQuery}`} 
                target="_blank" 
                rel="noopener noreferrer"
                className="font-medium text-sm md:text-base text-gray-100 hover:text-purple-300 transition duration-300"
              >
                {track.title}
              </a>
